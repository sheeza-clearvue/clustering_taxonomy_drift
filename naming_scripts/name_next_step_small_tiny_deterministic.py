...
"""
name_next_step_small_tiny_deterministic.py

Names SMALL/TINY next_step clusters with deterministic rules only.
...
FIELD_NAME = "next_step"
NEXT_STEP_COMPACT_ALIASES = {
    "dm": "DM",
    "dm name secured": "DM",
    "decision maker name secured": "DM",
    "dm name sec": "DM",
    "dm name sec email confirmed": "DM Email",
    ...
    "lo received": "LOA Received",
    "lo a received": "LOA Received",
    "lo sent": "LOA Sent",
    ...
    "contract date found": "Contract Date",
    "contract sent": "Contract Sent",
    "contract returned": "Contract Returned",
    "contract follow up": "Contract Follow Up",
    "contract followup": "Contract Follow Up",
    "contract chase": "Contract Chase",

    "awaiting customer": "Awaiting Customer",
    "awaiting customer documents": "Customer Docs",
    "awaiting customer documentation": "Customer Docs",
    "awaiting customer document": "Customer Docs",

    "awaiting supplier": "Awaiting Supplier",
    "awaiting supplier response": "Supplier Response",
    "awaiting supplier action": "Supplier Response",
    "awaiting supplier confirmation": "Supplier Response",
    "awaiting supplier update": "Supplier Response",

    "customer documents required": "Customer Docs",
    "customer to send documents": "Customer Docs",
    "customer to provide documents": "Customer Docs",
    "customer to return documents": "Customer Docs",

    "arrange new appointment": "New Appointment",
    "arrange new": "New Appointment",
    "arrange new time": "New Appointment",
    "arrange new date": "New Appointment",

    "outcome update": "Outcome Update",
    "nextstep neutral": "Outcome Update",
    "next step neutral": "Outcome Update",
    "next step positive": "Outcome Update",
    "positive outcome": "Outcome Update",
    "customer callback": "Customer Callback",
"customer contact required": "Customer Contact",
"customer supplier contact": "Supplier Contact",
"arrange new date time": "New Date Time",
}
...
NEXT_STEP_COMPONENT_ALIASES = {
    "dm name secured": "DM",
    "decision maker name secured": "DM",
    "dm name sec": "DM",
    "dm name sec email confirmed": "DM Email Confirmed",
    ...
    "awaiting customer documents": "Awaiting Customer Documents",
    "awaiting customer documentation": "Awaiting Customer Documents",
    "awaiting customer document": "Awaiting Customer Documents",

    "awaiting supplier response": "Awaiting Supplier Response",
    "awaiting supplier action": "Awaiting Supplier Response",
    "awaiting supplier confirmation": "Awaiting Supplier Response",
    "awaiting supplier update": "Awaiting Supplier Response",

    "customer to send documents": "Customer Documents Required",
    "customer to provide documents": "Customer Documents Required",
    "customer to return documents": "Customer Documents Required",

    "arrange new": "Arrange New Appointment",
    "arrange new time": "Arrange New Appointment",
    "arrange new date": "Arrange New Appointment",

    "nextstep neutral": "Outcome Update",
    "next step neutral": "Outcome Update",
    "next step positive": "Outcome Update",
    "positive outcome": "Outcome Update",
    "customer to call back": "Customer Callback",
"customer to call support": "Customer Callback",
"customer callback expected": "Customer Callback",

"customer to contact 111": "Customer Contact Required",
"customer to contact service centers": "Customer Contact Required",
"customer to contact agent": "Customer Contact Required",
"customer to contact agency": "Customer Contact Required",

"customer to call supplier": "Customer Supplier Contact",

"arrange new": "Arrange New Date Time",
"arrange new time": "Arrange New Date Time",
"arrange new date": "Arrange New Date Time",
    "contract followup": "Contract Follow Up",
}
# ---------------------------------------------------------------------
# DB helpers
...
def clean_next_step_component(raw_component: str) -> str:
    text = proper_case_name(raw_component)
    norm = normalize_for_name(text)

    if norm in NEXT_STEP_COMPONENT_ALIASES:
        return NEXT_STEP_COMPONENT_ALIASES[norm]

    return text


def canonical_next_step_sequence(seq: list[str]) -> list[str]:
    priority_index = {
        normalize_for_name(term): idx
        for idx, term in enumerate(NEXT_STEP_COMPONENT_PRIORITY)
    }
...
def next_step_sequence_display_name(seq: list[str]) -> str:
    ordered = canonical_next_step_sequence(seq)
    ...
    for item in ordered:
        norm = normalize_for_name(item)
        compact_parts.append(NEXT_STEP_COMPACT_ALIASES.get(norm, item))
...
def best_next_step_component_coverage_name(
    ...
    display_name = next_step_sequence_display_name(selected)
    ...
    selected_with_coverage = [
        ...
        for component in canonical_next_step_sequence(selected)
    ]
...
def extract_component_display_sequence(raw_label: str) -> list[str]:
    ...
    if FIELD_NAME == "next_step":
        displays = [clean_next_step_component(part) for part in parts]
        displays = [d for d in displays if d]
        return canonical_next_step_sequence(displays)
...
def best_component_signature_name(labels: list[dict[str, Any]], threshold: float) -> tuple[str | None, str | None]:
    ...
    if FIELD_NAME == "next_step":
        display_name = next_step_sequence_display_name(best_seq)
    else:
...
def deterministic_name(labels: list[dict[str, Any]]) -> tuple[str, str, str]:
    ...
    # next_step labels are set/combo labels like {A,B}.
    # Component signature must run first so reversed sets {A,B} and {B,A}
    # resolve to the same full action name instead of losing one component.
    if FIELD_NAME == "next_step":
        ...
        coverage_name, coverage_reason = best_next_step_component_coverage_name(
        labels_sorted,
        threshold=0.35,
         )
    ...
    # Safe dominant-label route for normal fields, or fallback for next_step
    # when no component signature has strong coverage.
    if top_display and top_weight / total >= 0.65 and is_acceptable_display_name(top_display):
        return (
            top_display,
    ...
    # For non-next_step fields, component signature stays after weighted mode.
    if FIELD_NAME != "next_step":
        sig_name, sig_reason = best_component_signature_name(labels_sorted, threshold=0.50)
        if sig_name and is_acceptable_display_name(sig_name):
            return (
...
def main() -> None:
    ...
    try:
        ...
        write_naming_output_file(
            output_path=f"taxonomy_cluster_output/next_step_small_tiny_names_{output_run_id}_{output_mode}.csv",
